...
describe('site e2e', () => {
  ...
  async function loadFlowPage(url: string): Promise<void> {
    ...
    await page.waitForSelector('#fy option', {timeout: NAVIGATION_TIMEOUT_MS});
    await page.waitForSelector('#hover_description', {timeout: NAVIGATION_TIMEOUT_MS});
    await page.waitForFunction(() => {
      const value = document.querySelector('#hover_description')?.textContent || '';
  ...
  async function loadTreePage(url: string): Promise<void> {
    await page.goto(url, {waitUntil: 'domcontentloaded', timeout: NAVIGATION_TIMEOUT_MS});
    await page.waitForSelector('#dropdown select', {timeout: NAVIGATION_TIMEOUT_MS});
    await page.waitForFunction(() => document.querySelectorAll('#treemap rect').length > 0, {
      timeout: NAVIGATION_TIMEOUT_MS,
    });
  }
  ...
  async function openFirstNavDropdown(): Promise<void> {
    ...
    await page.waitForFunction(() => Boolean(document.querySelector('nav .navbar-nav > li.dropdown.open')), {
      timeout: NAVIGATION_TIMEOUT_MS,
    });
  }
   ...
   * @returns {Promise<void>} Completion promise.
   */
  async function switchLegacyAccount(accountValue: 'Revenue' | 'Expense'): Promise<void> {
    await page.evaluate((nextValue) => {
      const selects = Array.from(document.querySelectorAll<HTMLSelectElement>('#dropdown select'));
  ...
  test('legacy overview/detail render in English with working dropdown and revenue switch', async () => {
    ...
    const initialRects = await page.$$eval('#treemap rect', (nodes) => nodes.length);
    expect(initialRects).toBeGreaterThan(0);

    await switchLegacyAccount('Revenue');
    const revenueRects = await page.$$eval('#treemap rect', (nodes) => nodes.length);
  ...
  test('legacy overview/detail render in Spanish with translated labels and revenue switch', async () => {
    ...
    const accountOptions = await page.$$eval('#dropdown select option', (nodes) => {
      return nodes.map((node) => ({
        text: (node.textContent || '').trim(),
        value: node.getAttribute('value') || '',
      }));
    });
    expect(accountOptions).toEqual(expect.arrayContaining([
