 ...
 * @returns {any} Function result.
 */
function buildRowChartData(entry: DiffEntry, years: SelectedYears, colors: Array<string>) {
  return {
    labels: [''],
    datasets: [
      {
...
export default class DiffTable extends React.Component<DiffTableProps, DiffTableState> {
    /**
   * Runs constructor.
   *
...
componentDidUpdate(prevProps: DiffTableProps): void {
    const isLimitedMode = Boolean(this.props.compactMode || this.props.constrainedMode);
    const wasLimitedMode = Boolean(prevProps.compactMode || prevProps.constrainedMode);
...
render(): React.JSX.Element {
    ...
    const diffEntries = getSortedBudgetKeys(this.props.data)
      ...
      .sort((left, right) => sortFunc(left[this.state.sortBy], right[this.state.sortBy]));
    ...
    return (
      <div>
            ...
            <tr>
              <th scope='col' colSpan={2} className='form-horizontal'>
                <div className='form-group'>
                  <label className='col-sm-3 col-sm-offset-6 control-label' htmlFor='diffSortControl'>
                    {t('diffTable.sortBy')}
                  </label>
        ...
        {isLimitedMode && diffEntries.length > defaultVisibleRows && !this.state.showAllRows ?
          (
            <button type='button' className='btn btn-default' onClick={this.handleShowAllRows}>
