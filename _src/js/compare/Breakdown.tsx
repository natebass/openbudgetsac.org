 ...
 */
function areSameYears(currentYears: SelectedYears, previousYears: SelectedYears): boolean {
  return currentYears.every((year, index) => {
    ...
    return year.fiscal_year_range === previousYear.fiscal_year_range &&
 ...
 * @returns {any} Function result.
 */
function hasCompleteYears(years: SelectedYears): years is [NonNullable<SelectedYears[0]>, NonNullable<SelectedYears[1]>] {
...
export default class Breakdown extends React.Component<BreakdownProps, BreakdownState> {
...
fetchData(years: SelectedYears): void {
    ...
    void fetchBreakdownData(yearNames, yearTypes, this.props.type, this.props.dimension)
      .then((budgets) => {
      ...
      .catch((error) => {
        ...
        this.setState({budgets: [{}, {}], error: t('error.breakdownUnavailable'), pending: false});
      });
  }
