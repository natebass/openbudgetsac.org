...
($ => {
  ...
  function safeNavigate(rawUrl) {
    try {
      ...
      if (parsed.protocol === 'http:' || parsed.protocol === 'https:') {
        // Legacy widget nodes may include external links. Allow only HTTP(S).
  ...
  OpenSpending.Treemap =
    ...
    function (elem, context, state) {
      ...
      if ($.browser.msie) {
        resources.push(
          OpenSpending.scriptRoot + '/widgets/treemap/js/excanvas.js',
        );
      }
      ...
      self.context = _.extend(
        {
          click: node => {
            if (node.data.link) {
              const link = context.embed
                ? node.data.link + '?embed=true'
                : node.data.link;
              safeNavigate(link);
            }
          },
          hasClick: node => node.data.link !== undefined,
          createLabel: (widget, domElement, node) => {
            if (node.data.value / self.total > 0.03) {
              domElement.innerHTML =
                "<div class='desc'><div class='amount'>" +
                escapeHtml(
                  OpenSpending.Utils.formatAmountWithCommas(
                    node.data.value,
                    0,
                    self.currency,
                  ),
          ...
          tooltipMessage: (widget, node) => {
            const percentualValue = (node.data.value * 100) / widget.total;
            return (
              node.name +
              ' (' +
              OpenSpending.Utils.formatAmountWithCommas(percentualValue, 2) +
              '%)'
            );
          },
          drilldown: node => {
      ...
      this.update =
        ...
        state => {
          self.state = state;
          self.state.drilldowns = self.state.drilldowns || [
            self.state.drilldown,
          ];
          ...
          if (
            typeof self.context.member !== 'undefined' &&
            typeof self.context.dimension !== 'undefined'
          ) {
      ...
      this.setNode =
        ...
        function (node) {
          ...
          self.data = {
            children: _.map(node.children, item => {
              if (item.color) {
        ...
        function () {
          ...
          self.tm = new $jit.TM.Squarified({
            ...
            Tips: {
              ...
              offsetX: 20,
              offsetY: 20,
              onShow: (tip, node, isLeaf, domElement) => {
                const html =
                  '<div class="tip-title">' +
            ...
            // Return a pruned subtree for the selected node.
            // This keeps legacy JIT drill-down behavior and callback contracts.
            request: (nodeId, level, onComplete) => {
              // Keep this hook for JIT compatibility.
              const tree = json;
