...
(function(global) {
   ...
   */
  function translateLegacyText(value: string | null | undefined, locale?: string) {
    if (value == null) {
      return value;
    }
   ...
   * @returns {void}
   */
  function applyLegacyAttributeTranslations(root: Document | Element, attributeMap?: Record<string, string>) {
    if (!attributeMap) {
      return;
    }
   ...
   * @returns {void}
   */
  function applyLegacyTextNodeTranslations(root: Document | Element, textMap?: Record<string, string>) {
   ...
   * @returns {void}
   */
  function propagateLocaleToLinks(locale: 'en-US' | 'es-419', root: Document | Element) {
  ...
  function init() {
    ...
    if (typeof document !== 'undefined') {
      document.querySelectorAll('[data-locale-selector]').forEach(function(select) {
        (select as HTMLSelectElement).addEventListener('change', function(event) {
          const nextLocale = event.target instanceof HTMLSelectElement ? event.target.value : DEFAULT_LOCALE;
