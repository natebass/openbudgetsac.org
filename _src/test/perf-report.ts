...
const bundlePath = path.resolve(__dirname, '../generated/js/dist/compare.bundle.js');
...
if (!fs.existsSync(bundlePath)) {
  fail([
    `Bundle does not exist: ${bundlePath}`,
...
console.log(`[perf] budgets: raw <= ${formatBytes(rawBudgetBytes)}, gzip <= ${formatBytes(gzipBudgetBytes)}`);

if (rawSize > rawBudgetBytes || gzipSize > gzipBudgetBytes) {
  const exceeded = [];
  if (rawSize > rawBudgetBytes) {
    exceeded.push(`raw exceeds budget by ${formatBytes(rawSize - rawBudgetBytes)}`);
  }
  if (gzipSize > gzipBudgetBytes) {
    exceeded.push(`gzip exceeds budget by ${formatBytes(gzipSize - gzipBudgetBytes)}`);
  }
